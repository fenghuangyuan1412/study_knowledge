...
1. **AI 软件测试**（batch-001 共 5 条）：B站「黑马 AI+软件测试」第 8 集被测项目分析 + 黑马头条实操笔记 + AI 测试概念全景 + 测试学习路线 + AI 生成用例趋势综述。
2. **毛选**（batch-001 共 5 条）：实践论 / 矛盾论 / 论持久战 / 反对本本主义 / 为人民服务 —— 原文要点 + 解读。

> 每个知识库 = localbrain 一份独立数据实例（AI 软件测试 → `~/.knowledge-base`；毛选 → `~/.knowledge-base-maoxuan`），Web 页面顶部下拉即可切换。
