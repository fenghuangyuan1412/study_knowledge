...
| 批次 | 主题 | 状态 |
| --- | --- | --- |
| batch-001 | 被测项目认知 + AI测试全景 + 入门路线（首航 5 条） | 已入库 |
| batch-002 | 测试开发能力体系（参照华测《测试开发大师课》大纲，5 条） | 已入库 |

> **学习路线的两段**：batch-001 回答「测试是什么、AI 在往哪走」；batch-002 回答「往测试开发走，具体要会什么、按什么顺序学」——
> 后者的三级跃迁（会测 → 会自动化 → 会平台化/工程化）可作为长期自查清单。
