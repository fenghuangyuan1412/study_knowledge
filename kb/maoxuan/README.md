...
| batch-002 | 第一卷全卷内容总结（18 篇，本地模型 map-reduce 生成） | 已入库 |

> **两层互补**：batch-001 是**精读层**（5 篇代表作的背景+要点+方法论解读）；batch-002 是**通读层**（第一卷全篇「这篇讲了什么」）。
> 问「实践论讲了什么」两层都能答；问「毛选里还有哪些篇讲调查研究」则靠 batch-002 的全卷覆盖。
